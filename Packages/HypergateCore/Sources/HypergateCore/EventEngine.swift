...
public struct EventQuery: Codable, Sendable, Equatable {
    ...
    public init(from: Date, to: Date, bodies: [Body] = Body.allCases, types: [EventKind] = EventKind.standard) throws {
        guard from.timeIntervalSince1970.isFinite, to.timeIntervalSince1970.isFinite,
            to > from, to.timeIntervalSince(from) <= 90 * 86400, !bodies.isEmpty, !types.isEmpty else {
            throw CoreError.invalidArgument("Choose bodies/types and an increasing time range of at most 90 days.")
        }
...
public struct EventDocument: Codable, Sendable {
    ...
    public init(provider: ProviderMetadata, calculatedAt: Date, query: EventQuery, events: [AstroEvent]) {
        schemaVersion = 1; self.provider = provider; self.calculatedAt = calculatedAt
        self.query = query; coverageEnd = query.to; complete = true; self.events = events
    }
}
...
public actor EventEngine {
    ...
    public init(provider: any EphemerisProvider, evaluationBudget: Int = 500_000, clock: any WallClock = SystemWallClock()) {
        self.provider = provider; self.evaluationBudget = evaluationBudget; self.clock = clock
    ...
    public func events(_ query: EventQuery) async throws -> EventDocument {
        ...
        return EventDocument(provider: provider.metadata, calculatedAt: clock.now(), query: query, events: events)
    }
}
...
private actor EphemerisCache {
    ...
    func at(_ date: Date) async throws -> [Body: Position] {
        ...
        guard positions.count == Body.allCases.count, Set(positions.map(\.body)).count == positions.count,
            positions.allSatisfy({ $0.longitude.isFinite && $0.latitude.isFinite && $0.velocity.isFinite }) else {
...
private enum SearchSignal: Sendable {
    case longitude(Body), separation(Body, Body), velocity(Body)
...
private actor EventSearch {
    ...
    init(cache: EphemerisCache, metadata: ProviderMetadata, budget: Int) {
        self.cache = cache; self.metadata = metadata; self.budget = budget
    }
    func position(_ body: Body, _ date: Date) async throws -> Position {
        ...
        guard let position = try await cache.at(date)[body] else { throw CoreError.provider("Missing body.") }
        return position
    }
    func sample(_ signal: SearchSignal, _ date: Date) async throws -> SignalSample {
        ...
        case .separation(let a, let b):
            ...
            return SignalSample(value: Angle.normalized(pa.longitude - pb.longitude), derivative: pa.velocity - pb.velocity)
        case .velocity(let body):
            let p = try await position(body, date)
            let left = max(metadata.earliest, date.addingTimeInterval(-300))
            let right = min(metadata.latestExclusive.addingTimeInterval(-0.001), date.addingTimeInterval(300))
            let a = try await position(body, left)
            let b = try await position(body, right)
            return SignalSample(value: p.velocity, derivative: (b.velocity - a.velocity) / right.timeIntervalSince(left) * 86400)
        }
    }
    func run(_ query: EventQuery) async throws -> [AstroEvent] {
        ...
        while day < query.to {
            ...
            let end = min(day.addingTimeInterval(86400), metadata.latestExclusive.addingTimeInterval(-0.001))
            var candidates: [Candidate] = []
            for body in query.bodies {
                if query.types.contains(.ingress) {
                    candidates += try await search(.longitude(body), kind: .ingress, targets: (0..<12).map { Double($0) * 30 }, start: start, end: end)
                }
                if body.canRetrograde, query.types.contains(where: \.isStation) {
                    candidates += try await search(.velocity(body), kind: .stationDirect, targets: [0], start: start, end: end)
                }
            }
            for a in query.bodies.indices {
                for b in query.bodies.indices where b > a {
                    for kind in query.types where !kind.angles.isEmpty {
                        candidates += try await search(.separation(query.bodies[a], query.bodies[b]), kind: kind, targets: kind.angles, start: start, end: end)
                    }
                }
            }
            ...
            for candidate in candidates.sorted(by: { $0.instant < $1.instant }) {
                ...
                let afterTime = min(metadata.latestExclusive.addingTimeInterval(-0.001), candidate.instant.addingTimeInterval(delta))
                ...
                let semantic = candidate.signal.bodies.map(\.rawValue).joined(separator: "+") + ":" + kind.rawValue + ":" + String(Int(candidate.target))
                // Duplicate endpoint discoveries share the same root; distinct refined roots
                // remain separate. Identity never hashes a floating-point timestamp.
                if let prior = seen[semantic], abs(prior.timeIntervalSince(candidate.instant)) < 0.01 { continue }
                seen[semantic] = candidate.instant
                ...
                result.append(AstroEvent(id: id, instant: candidate.instant, kind: kind, bodies: candidate.signal.bodies,
    ...
    func search(_ signal: SearchSignal, kind: EventKind, targets: [Double], start: Date, end: Date) async throws -> [Candidate] {
        var found: [Candidate] = []
        var a = start
        while a < end {
    ...
    func segment(_ signal: SearchSignal, kind: EventKind, targets: [Double], a: Date, b: Date, depth: Int) async throws -> [Candidate] {
        ...
        if signal.periodic && maximumSpeed * duration / 86400 > 15 {
            ...
            let left = try await segment(signal, kind: kind, targets: targets, a: a, b: midpoint, depth: depth + 1)
            return try await left + segment(signal, kind: kind, targets: targets, a: midpoint, b: b, depth: depth + 1)
        }
        ...
        for i in 0..<(times.count - 1) {
            let left = times[i]; let right = times[i + 1]
            ...
            for target in targets {
                ...
                let low = min(fa, unwrappedB); let high = max(fa, unwrappedB)
                guard branch >= low - 1e-10, branch <= high + 1e-10 else { continue }
                let root: Date
                if abs(fa - branch) <= 1e-10 { root = left }
                else if abs(unwrappedB - branch) <= 1e-10 { root = right }
                else {
                ...
                if root >= a && root < b { found.append(Candidate(instant: root, target: target, signal: signal, kind: kind)) }
            }
        }
        return found
    }

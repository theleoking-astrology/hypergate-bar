...
public struct AstronomyEngineProvider: EphemerisProvider {
  public let metadata = ProviderMetadata(
    ...
    convention:
      "tropical-geocentric-true-ecliptic-of-date; light-time+aberration; no gravitational deflection; upstream delta-T; v1",
    earliest: Date(timeIntervalSince1970: 946_684_800),
    latestExclusive: Date(timeIntervalSince1970: 2_556_144_000)
  )
  public init() {}
  ...
  /// Diagnostic exposure of the provider's own UTC/UT-to-TT model, for accuracy
  /// decomposition. This does not change the model or apply another correction.
  public func ttMinusUTCSeconds(at date: Date) async throws -> Double {
    try metadata.validate(date)
    try Task.checkCancellation()
    return await AstronomyExecutor.shared.ttMinusUTCSeconds(at: date)
  }
}
...
private actor AstronomyExecutor {
  ...
  func ttMinusUTCSeconds(at date: Date) -> Double {
    let time = Astronomy_TimeFromDays((date.timeIntervalSince1970 - 946_728_000) / 86400)
    return (time.tt - time.ut) * 86400
  }
  ...
  private func coordinates(_ body: Body, _ date: Date) throws -> (Double, Double) {
    let days = (date.timeIntervalSince1970 - 946_728_000) / 86400
    let time = Astronomy_TimeFromDays(days)
    var vector = Astronomy_BackdatePosition(time, BODY_EARTH, bodyCode(body), ABERRATION)
    guard vector.status == ASTRO_SUCCESS else {
      throw CoreError.provider(
        "C vector calculation failed for \(body.name): \(vector.status.rawValue)")
    }
    vector.t = time
  ...
  func positions(at date: Date, bodies: [Body]) throws -> [Position] {
    try bodies.map { body in
      ...
      guard abs(coarse - fine) < 0.00001 else {
        throw CoreError.provider("Longitudinal derivative did not converge for \(body.name).")
      }

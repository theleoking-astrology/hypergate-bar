...
struct SyntheticProvider: EphemerisProvider {
    ...
    let metadata = ProviderMetadata(name: "Synthetic", revision: "1", convention: "analytic",
                                    earliest: Date(timeIntervalSince1970: 0), latestExclusive: Date(timeIntervalSince1970: 8640000))
    func positions(at date: Date, bodies: [Body]) async throws -> [Position] {
        ...
        return try bodies.map { body in
            ...
            return try Position(body: body, longitude: longitude, latitude: 0, velocity: hourlyVelocity * 24)
        }
    }
}
...
@Test func findsTwoIngressesWithSameEndpointSigns() async throws {
    ...
    let query = try EventQuery(from: epoch, to: epoch.addingTimeInterval(6 * 3600), bodies: [.mercury], types: [.ingress])
...
@Test func classifiesStationByDerivativeSign() async throws {
    ...
    let engine = EventEngine(provider: provider)
    let query = try EventQuery(from: epoch, to: epoch.addingTimeInterval(6 * 3600), bodies: [.mercury], types: [.stationRetrograde, .stationDirect])
    let events = try await engine.events(query).events
...
@Test func repeatedAspectsAndOverlappingQueriesKeepIdentities() async throws {
    ...
    let engine = EventEngine(provider: provider)
    let whole = try await engine.events(EventQuery(from: epoch, to: epoch.addingTimeInterval(6 * 3600), bodies: [.sun, .mars], types: [.square]))
    let overlapping = try await engine.events(EventQuery(from: epoch.addingTimeInterval(3600), to: epoch.addingTimeInterval(4 * 3600), bodies: [.sun, .mars], types: [.square]))
...
@Test func boundaryIsHalfOpenAndWrapsPiscesToAries() async throws {
    ...
    let a = try await engine.events(EventQuery(from: epoch, to: boundary, bodies: [.moon], types: [.ingress]))
    let b = try await engine.events(EventQuery(from: boundary, to: epoch.addingTimeInterval(7200), bodies: [.moon], types: [.ingress]))
...
@Test func ingressTangencyDoesNotChangeSign() async throws {
    ...
    let engine = EventEngine(provider: provider)
    let result = try await engine.events(EventQuery(from: epoch, to: epoch.addingTimeInterval(4 * 3600), bodies: [.mercury], types: [.ingress]))
    #expect(result.events.isEmpty)
}

@Test func budgetAndInvalidRangeFailExplicitly() async throws {
    #expect(throws: CoreError.self) { try EventQuery(from: epoch, to: epoch.addingTimeInterval(91 * 86400)) }

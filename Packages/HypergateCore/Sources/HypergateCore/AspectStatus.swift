...
public struct AspectStatus: Sendable, Identifiable {
  ...
  public static func current(
    positions: [Position], types: [EventKind], majorOrb: Double = 3, minorOrb: Double = 1
  ) -> [AspectStatus] {
    ...
    for i in positions.indices {
      for j in positions.indices where j > i {
        ...
        for type in types where !type.angles.isEmpty {
          ...
          guard
            let error = type.angles.map({ Angle.difference(separation, $0) }).min(by: {
              abs($0) < abs($1)
            })
          else { continue }
          ...
          let trend: Trend =
            abs(error) < 1e-6
            ? .exact : abs(rate) < 1e-7 ? .turning : error * rate < 0 ? .applying : .separating
          result.append(
            AspectStatus(
              bodies: [a.body, b.body], kind: type, orb: abs(error), allowedOrb: allowed,
              trend: trend))
        }
      }
    }

  ...
  public static func parse(_ text: String) throws -> Date {
    ...
    guard let match = regex.firstMatch(in: text, range: NSRange(location: 0, length: ns.length))
      ...
      throw CoreError.invalidArgument(
        "Use an ISO 8601 date with seconds and an explicit offset or Z.")
    }
    ...
    guard let utc = TimeZone(secondsFromGMT: 0) else {
      throw CoreError.invalidArgument("UTC unavailable.")
    }
    calendar.timeZone = utc
    let parts = DateComponents(
      year: year, month: month, day: day, hour: hour, minute: minute, second: second)
    guard let civil = calendar.date(from: parts),
      calendar.dateComponents([.year, .month, .day, .hour, .minute, .second], from: civil) == parts
    else {

...
public enum EventKind: String, CaseIterable, Codable, Sendable, Identifiable {
  case ingress, square, opposition
  case stationRetrograde = "station_retrograde"
  case stationDirect = "station_direct"
  ...
  public static let standard: [EventKind] = [
    .ingress, .square, .opposition, .stationRetrograde, .stationDirect,
  ]
}
...
public struct AstroEvent: Codable, Sendable, Equatable, Identifiable {
  ...
  public init(
    id: String, instant: Date, kind: EventKind, bodies: [Body], longitude: Double? = nil,
    ...
    self.previousSign = previousSign
    self.enteredSign = enteredSign
  }
...
public enum IngressSearch {
  public static func next(body: Body, after start: Date, provider: any EphemerisProvider)
    async throws -> AstroEvent
  {
    var left = start
    ...
    for _ in 0..<360 {
      ...
      if current.sign != previous.sign {
        ...
        return AstroEvent(
          id: "v1:\(body.rawValue):ingress:\(day):\(after.sign.rawValue)",
  ...
  private static func position(_ body: Body, _ date: Date, _ provider: any EphemerisProvider)
...
public enum RootRefinement {
  public static func solve(
    ...
    value: @Sendable (Date) async throws -> Double
  ) async throws -> Date {
    var a = start
    var b = end
